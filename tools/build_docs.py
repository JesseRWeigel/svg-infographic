...
Out:  examples/*.svg  and  docs/index.html
"""
...
<h2>Three things the browser changed about this engine</h2>
<p>All three were found by measuring the published page, not by reading the code. Each one is a
case where the engine was correct about the font and wrong about the renderer.</p>
<div class="tablewrap"><table>
<thead><tr><th>what was wrong</th><th class="num">error</th><th>the fix</th></tr></thead>
<tbody>
<tr><td>Chromium rounded every glyph advance to a whole pixel, so a 45-character line rendered
wider than the sum of its advances</td><td class="num">12.05 px</td>
<td><code>text-rendering: geometricPrecision</code>, after which the two agree to 0.014 px</td></tr>
<tr><td><code>font-kerning="none"</code> as an SVG attribute was ignored; computed style came back
<code>auto</code></td><td class="num">n/a</td>
<td>set it in a <code>style</code> attribute, where it takes effect</td></tr>
<tr><td>the box the renderer reports covers the font's full line metrics, not the typographic
ascender and descender, so glyphs sat outside their containers</td><td class="num">518 elements</td>
<td>reserve the union of the hhea, typographic and windows metrics, plus the ink of the glyphs
being set, plus one pixel for the renderer's rounding</td></tr>
</tbody></table></div>
<p>A fourth was found by attacking the verify script rather than the engine: inverting one
rounding function so measured widths rounded down instead of up produced containers a hundredth
of a pixel narrower than their content, and the checker's tolerance was wide enough to swallow it.
The tolerance is now 0.001 px and the attack fails as it should.</p>
...
<footer>
<p>Part of the 722 things to build catalog, task MEDIA-031. Fonts embedded as subset WOFF2: DejaVu Sans
...
(function(){
  ...
  function browserCheck(){
    ...
    var res = {
      ...
      minSepWhere: "", boxes: 0,
      inkOverflows: 0, inkWorst: Infinity, inkPairs: 0, inkCollisions: 0, inkMinSep: Infinity
    };
    var perSvg = new Map();
    var inkSvg = new Map();
    for (var i = 0; i < texts.length; i++){
      ...
      if (content.trim() !== "" && len > 0){
        ...
        // Second pass on the renderer's own ink box. The container is widened by the run's
        // declared side-bearing overhang and one pixel for the outward quantization Chromium
        // applies to the box it reports.
        var bb = null;
        try { bb = t.getBBox(); } catch (e) { bb = null; }
        if (bb && bb.width > 0){
          var ink = (t.getAttribute("data-ink") || "0 0").trim().split(/\s+/).map(Number);
          var bl = left - ink[0] - 1.0, br = right + ink[1] + 1.0;
          var bm = [bb.x - bl, br - (bb.x + bb.width), bb.y - top, bottom - (bb.y + bb.height)];
          for (var q = 0; q < bm.length; q++){
            if (bm[q] < -TOL) res.inkOverflows++;
            if (bm[q] < res.inkWorst) res.inkWorst = bm[q];
          }
          if (!inkSvg.has(svg)) inkSvg.set(svg, []);
          inkSvg.get(svg).push({x0: bb.x, x1: bb.x + bb.width, y0: bb.y, y1: bb.y + bb.height});
        }
      }
    }
    ...
    inkSvg.forEach(function(runs){
      for (var a = 0; a < runs.length; a++){
        for (var b = a + 1; b < runs.length; b++){
          var p = runs[a], q = runs[b];
          var sep = Math.max(Math.max(p.x0 - q.x1, q.x0 - p.x1), Math.max(p.y0 - q.y1, q.y0 - p.y1));
          res.inkPairs++;
          if (sep <= 0) res.inkCollisions++;
          if (sep < res.inkMinSep) res.inkMinSep = sep;
        }
      }
    });
  ...
  function render(){
    ...
    var rows = [
      ...
      ["ink boxes outside their container (getBBox)", "&mdash;", r.inkOverflows],
      ["worst ink-box clearance (px)", "&mdash;", n(r.inkWorst, 4)],
      ["intersecting ink boxes", "&mdash;", r.inkCollisions],
      ["minimum ink-box separation (px)", "&mdash;", n(r.inkMinSep, 4)]
    ];
    ...
    var good = r.overflows === 0 && r.collisions === 0 && r.worst >= -0.05 &&
               r.worstDelta < 0.05 && r.inkOverflows === 0 && r.inkCollisions === 0;
    document.getElementById("verdict").innerHTML = good
      ? '<span class="ok">This browser measured ' + r.elements + " text elements and " + r.pairs +
        " label pairs, twice over: once by advance width and once by the renderer's own ink box. " +
        "None overflowed, none intersected, and the widest disagreement between the engine's " +
        "font-metric arithmetic and the browser's text engine was " + n(r.worstDelta, 4) + "px.</span>"
  ...
  // Two probes on purpose. The engine's arithmetic matches the renderer only when the
  // renderer is told to stop hinting glyph advances to whole pixels, and showing both makes
  // that visible instead of asserted: the "default" column comes out on integers.
  var probe = null;
  function browserWidth(s, size, weight, precise){
    if (!probe){
    ...
    t.setAttribute("style",
      (precise ? "text-rendering:geometricPrecision;" : "text-rendering:auto;") +
  ...
  function runLab(){
    ...
    var worst = Infinity, worstDelta = 0, worstNaive = 0;
    var rows = lines.map(function(ln, i){
      var e = measure(tbl, ln, size).w;
      var bp = browserWidth(ln, size, weight, true);
      var bd = browserWidth(ln, size, weight, false);
      var slack = avail - e;
      if (slack < worst) worst = slack;
      worstDelta = Math.max(worstDelta, Math.abs(bp - e));
      worstNaive = Math.max(worstNaive, Math.abs(bd - e));
      return "<tr><td class=\"num\">" + (i + 1) + "</td><td><code>" +
        ln.replace(/&/g, "&amp;").replace(/</g, "&lt;") + "</code></td>" +
        '<td class="num">' + n(e, 2) + '</td>' +
        '<td class="num">' + n(bp, 2) + '</td>' +
        '<td class="num">' + n(bd, 2) + '</td>' +
        '<td class="num ' + (slack < 0 ? "bad" : "ok") + '">' + n(slack, 2) + "</td></tr>";
    }).join("");

    out.innerHTML =
      '<div class="tablewrap"><table><thead><tr><th>line</th><th>content</th>' +
      '<th class="num">engine (px)</th><th class="num">browser, precise (px)</th>' +
      '<th class="num">browser, default (px)</th>' +
      ...
      "The engine and the browser disagree by at most " + n(worstDelta, 4) +
      "px once the renderer is told not to hint advances to whole pixels, and by " +
      n(worstNaive, 2) + "px if it is not." +
...
def main() -> int:
    ...
    print(f"wrote {len(svgs)} svg files to examples/")
